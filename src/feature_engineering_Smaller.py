"""removed quite some features to make it smaller
features removed where related for example to competitor history 
added fourrier
changed relevance score to 5*booking or 1*click
"""
...
class EnhancedFeatureEngineer:
    ...
    def _create_temporal_features(self, df):
        """Create time-based with fourrier features"""
        df['date_time'] = pd.to_datetime(df['date_time'])
        df['hour_sin'] = np.sin(df['date_time'].dt.hour * (2 * np.pi / 24))
        df['hour_cos'] = np.cos(df['date_time'].dt.hour * (2 * np.pi / 24))
        df['day_of_week_sin'] = np.sin(df['date_time'].dt.dayofweek * (2 * np.pi / 7))
        df['day_of_week_cos'] = np.cos(df['date_time'].dt.dayofweek * (2 * np.pi / 7))
        df['month_sin'] = np.sin(df['date_time'].dt.month * (2 * np.pi / 12))
        df['month_cos'] = np.cos(df['date_time'].dt.month * (2 * np.pi / 12))
    ...
    def _create_user_features(self, df):
        ...
        # If has_booking_history exists, use it; otherwise set to 0
        if 'has_booking_history' not in df.columns:
            df['has_booking_history'] = 0
        
        # Create simplified user features without requiring historical data
        df['user_search_count'] = df.groupby('visitor_location_country_id')['srch_id'].transform('count')
        df['user_destination_count'] = df.groupby(['visitor_location_country_id', 'srch_destination_id'])['srch_id'].transform('count')
    ...
    def _create_relevance_score(self, df):
        """Create a relevance score target (5*booking, or! 1*click)"""
        # This is a common approach in learning-to-rank problems
        if 'booking_bool' in df.columns and 'click_bool' in df.columns:
            df['relevance_score'] = 5 * df['booking_bool']
        else:
            df['relevance_score'] = 1 * df['click_bool']
    ...
    def create_enhanced_features(self, df, is_training=True):
        ...
        cols_to_drop = [
            'date_time',  # Original datetime
            'visitor_hist_starrating',  # High missing rate
            'visitor_hist_adr_usd',     # High missing rate
            'prop_starrating_monotonic', # Redundant
            'prop_review_monotonic',     # Redundant
            'prop_location_monotonic',   # Redundant
            'prop_historical_ctr_log',   # Keep non-log version
            'prop_historical_br_log',    # Keep non-log version
            'prop_price_std',           # High missing rate
            'prop_position_std',        # High missing rate
            'random_bool',              # Not useful
            'dest_month'                # High memory usage
        ]
